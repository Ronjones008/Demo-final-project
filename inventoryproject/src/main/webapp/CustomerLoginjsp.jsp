...
<h1>CUSTOMER LOGIN PAGE</h1>
	...
	<form method="post" style="width: 100%;">
    		...
    		<label style="width:27%">User Name :</label>
            	<input style="width:70%" class="form-control" type="text" name="userName" placeholder="User Name" required>
            </div>
            <div class="mb-3 formI">
            <label style="width:27%">Password :</label>
            <input style="width:70%" class="form-control" type="password" name="passCode" placeholder="Password" required>
            </div>
...
<br>
      ...
      <c:if test="${param.userName != null && param.passCode != null}">
       ...
       <c:if test="${dao.authenticate(param.userName, param.passCode) == 'login' }">
       		<c:set var="userName" value="${param.userName}" scope="session"></c:set>
            <c:redirect url="CustomerMenu.jsp"/>
        </c:if>
       <c:if test="${dao.authenticate(param.userName, param.passCode) == 'reset' }">
       		<c:set var="userName" value="${param.userName}" />
            <c:redirect url="Reset.jsp?name=${userName}"/>
        </c:if>
        <c:if test="${dao.authenticate(param.userName, param.passCode) == null }">
       <p class="text-danger" style="text-align: center"><b>Invalid Credentials</b></p>
